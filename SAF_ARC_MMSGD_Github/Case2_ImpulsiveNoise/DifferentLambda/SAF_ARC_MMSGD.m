...
lamda = 0.995;
for n = 2:length(emf)
    emf(n) = lamda*emf(n-1) + (1-lamda)*edb(n);
end
noiseLevel(1: length(emf)) = -snr;

figure; hold on; grid on; box on; %ylim([-snr-5, 10]);
set(gca, 'FontSize', 14, 'FontWeight', 'demi');
plot(emf, 'Color', [1 0 0], 'LineWidth', 2); plot(noiseLevel, '--', 'Color', [0 0 1], 'LineWidth', 2);
title('Wiener SAF Convergence Test'); xlabel('Samples'); ylabel('MSE/dB 10lg({\itJ})'); legend('MSE', 'NoiseLevel');
%% Plot
Co = linspecer(3);
...
xlabel('n'); ylabel('Output'); title('Adaptive Spline Filtering');
legend('desired d', 'filtered y', 'error e', 'noise n', 'location', 'best');

figure; hold on; grid on; box on;
set(gca, 'fontname', 'Times New Roman', 'fontweight', 'bold', 'fontsize', 20);
plot(w0, 'b', 'linewidth', 3); plot(w, 'r--', 'linewidth', 3); title('\bfLinear Weights');
l1 = legend(' desired \bfw', ' filtered \bfw', 'location', 'best');
set(l1, 'fontsize', 16, 'location', 'southeast', 'box', 'off');
...
xlabel('s'); ylabel('y'); title('\bfSpline Control Points');
l2 = legend(' desired \bfq', ' filtered \bfq', ' initialization', 'location', 'southeast');
set(l2, 'fontsize', 16, 'location', 'southeast', 'box', 'off');
%% Error Estimate
cem = em(9/10*end:end);
msgd_mean = mean(cem);
msgd_std = std(cem);
