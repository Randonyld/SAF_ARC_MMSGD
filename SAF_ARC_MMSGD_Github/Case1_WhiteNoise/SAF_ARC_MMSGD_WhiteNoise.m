...
for k = 1:ntrial
    ...
    if ntrial == 1
        figure;
        subplot(311); plot(x); xlabel('n'); ylabel('x(n)'); title('System Input x(n)'); grid on;
        subplot(312); plot(s0); xlabel('n'); ylabel('s(n)'); title('Linear Output s(n)'); grid on;
        subplot(313); plot(d); xlabel('n'); ylabel('d(n)'); title('System Output d(n)'); grid on;
    end
    %% Adaptive Filtering
...
figure; hold on; grid on; box on;
set(gca, 'FontSize', 15, 'FontWeight', 'bold');
plot(w0, 'b', 'linewidth', 4); plot(w, 'r--', 'linewidth', 3);
xlabel(' '); ylabel('\bfw'); title('\bfLinear Filter Weights');
text(1.2, 0.7, '({\bf\ita})', 'fontsize', 18);
l1 = legend(' desired \bfw', ' adapted \bfw', 'location', 'best');
...
set(gca, 'FontSize', 15, 'FontWeight', 'bold');
plot(xx0, yy0, 'b', 'linewidth', 4); plot(xx, yy, 'r--', 'linewidth', 3); plot(qx, qx, 'k--', 'linewidth', 1);
xlabel('Nonlinear input {\its}({\itn})'); ylabel('Nonlinear output {\ity}({\itn})'); title('\bfProfile of spline nonlinearity');
text(-2, 1.9, '({\bf\itb})', 'fontsize', 18);
l2 = legend(' desired', ' adapted', ' initialization', 'location', 'southeast');
...
%% Save
emf_safarcmmsgd = emf;
save safarcmmsgd.mat emf_safarcmmsgd msgd_mean msgd_std