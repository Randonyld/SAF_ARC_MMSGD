...
for k = 1:ntrial
    ...
    for n = M:N
        ...
        w = w + rate_w*ed/dx*dU'*C*Qi*Xn; % update w
        qy(ind-1:ind+2) = qy(ind-1:ind+2) + rate_q*ed*C'*U; % update qy
    end
    em = em + (e - noisei).^2;
end
%% Learning Curve
...
disp('>>> SAF-ARC algorithm end.');
%% Save
emf_safarc = emf;
save safarc.mat emf_safarc arc_mean arc_std