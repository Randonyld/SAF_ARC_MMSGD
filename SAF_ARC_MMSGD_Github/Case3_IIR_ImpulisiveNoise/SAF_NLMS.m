...
for k = 1:ntrial
    ...
    for n = M:N
        ...
        w = w + rate_w*e(n)/(1e-3 + U'*U)/dx*dU'*C*Qi*Xn; % update w
        qy(ind-1:ind+2) = qy(ind-1:ind+2) + rate_q*e(n)/(1e-3 + U'*U)*C'*U; % update qy
    end
    t = toc;
    time0 = time0 + t;
    em = em + (e - noisei).^2;
...
disp('>>> SAF-NLMS algorithm end.');
%% Save
emf_safnlms = emf;
save safnlms.mat emf_safnlms nlms_mean nlms_std

if ntrial == 1
    wnlms = w;
    qxnlms = xx;
    qynlms = yy;
    save nlms.mat wnlms qxnlms qynlms
end